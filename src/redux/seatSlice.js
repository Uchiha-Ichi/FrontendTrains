...
export const fetchSeat = createAsyncThunk(
  ...
  async ({ tripId, from, to }, { rejectWithValue }) => {
    try {
      ...
      const response = await axios.post("http://localhost:8080/api/carriages/seats",
        {
          tripId: tripId,
      ...
      // console.log(response.data.carriages)
...
const seatSlice = createSlice({
  ...
  reducers: {
    selectSeat: (state, action) => {
      const { seatId, ticketPrice, reservation } = action.payload; // seatId và ticketPrice từ action

      // Kiểm tra xem ghế đã được chọn chưa
      const seatIndex = state.selectedSeats.findIndex(seat => seat.seatId === seatId);
      ...
      if (seatIndex !== -1) {
        ...
        state.selectedSeats.push({ seatId, ticketPrice, reservation });
        state.totalPrice += ticketPrice;
      }
    },
